...
describe('convertBrowserCoordinates', () => {
  it('still returns coordinates if arguments are ommitted / undefined', () => {
    const results = convertBrowserCoordinates()
    expect(results).toBeDefined()
    expect(results.lat).toBeDefined()
    expect(results.lng).toBeDefined()
    expect(typeof results.lat).toEqual('number')
    expect(typeof results.lng).toEqual('number')
  })
